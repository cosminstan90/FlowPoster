...
Required cms_config keys:
    api_url          — DreamCMS publisher endpoint,
                       e.g. https://candvisam.ro/api/publisher/receive
    publisher_token  — Value of CMS_PUBLISHER_TOKEN in the DreamCMS .env
    site_id          — (optional) target site ID; omit to use the site matched
                       by the incoming domain
"""
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                return ConnectionResult(
                    success=data.get("success", True),
                    message=data.get("message", "DreamCMS connection OK"),
                )
